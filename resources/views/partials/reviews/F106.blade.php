    ...
    @if(!empty($review->json->correctExpectedCostComment) || !empty($review->json->correctExpectedCost))
        <div class="reviews__body__one">
            ...
            @if(!empty($review->json->correctExpectedCostComment))
                <p><span class="{{ $review->json->correctExpectedCost }}">{{ $review->json->correctExpectedCost=='yes'?'ТАК':'НІ' }}</span>, {!! nl2br(trim(strip_tags($review->json->correctExpectedCostComment))) !!}</p>
            @endif
        </div>
    @endif
</div>