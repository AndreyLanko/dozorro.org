    ...
    @if(!empty($review->json->impartialProductRequirementsComment) || !empty($review->json->impartialProductRequirements))
        <div class="reviews__body__one">
            ...
            @if(!empty($review->json->impartialProductRequirementsComment))
                <p><span class="{{ $review->json->impartialProductRequirements }}">{{ $review->json->impartialProductRequirements=='yes'?'ТАК':'НІ' }}</span>, {!! nl2br(trim(strip_tags($review->json->impartialProductRequirementsComment))) !!}</p>
            @endif
        </div>
    @endif
</div>